...
class TriangularObstacle(object):
  # generate triangle obstacles object on simulated environment
  ...
  def contains(self, x, y):
    # check if a point is located within an obstacle
...
class Environment(object):
  ...
  def check_collision_line(self, start, end, num_steps=100):
    # Checks if the line segment between 'start' and 'end' collides with any obstacles via Linear Interpolation
  ...
  def plot_query(self, x_start, y_start, x_goal, y_goal):
    start_mark, = pl.plot([x_start], [y_start], "bs", markersize = 8)
    goal_mark, = pl.plot([x_goal], [y_goal], "y*", markersize = 12)
    return [start_mark, goal_mark]
